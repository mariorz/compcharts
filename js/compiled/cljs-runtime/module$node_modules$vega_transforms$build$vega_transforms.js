...
count:counts};if(!countOnly)for(var n$jscomp$0=measures.length,a=cell.agg=Array(n$jscomp$0),i$jscomp$0=0;i$jscomp$0<n$jscomp$0;++i$jscomp$0)a[i$jscomp$0]=new measures[i$jscomp$0](cell);if(cell.store)var store=cell.data=new TupleStore;cell.add=function(t){cell.num+=1;if(!countOnly){store&&store.add(t);for(var i$425=0;i$425<n$jscomp$0;++i$425)a[i$425].add(a[i$425].get(t),t)}};cell.rem=function(t){--cell.num;if(!countOnly){store&&store.rem(t);for(var i$426=0;i$426<n$jscomp$0;++i$426)a[i$426].rem(a[i$426].get(t),
t)}};cell.set=function(t){var n;store&&store.values();var i=0;for(n=counts.length;i<n;++i)t[counts[i]]=cell.num;if(!countOnly)for(i=0,n=a.length;i<n;++i)a[i].set(t)};cell.init=function(){cell.num=0;store&&store.reset();for(var i$427=0;i$427<n$jscomp$0;++i$427)a[i$427].init()};return cell}function Window(params){vegaDataflow.Transform.call(this,{},params);this._mlen=0;this._mods=[]}function processPartition(list,state,cmp,_){var sort=_.sort,range=sort&&!_.ignorePeers;_=_.frame||[null,0];list=list.data(cmp);
...
max=-Infinity,i=0,j;i<n;++i){var g=groups[i].sort(sort);j=-1;for(var $jscomp$iter$103=$jscomp.makeIterator(vegaStatistics.dotbin(g,source,smooth,field)),$jscomp$key$v=$jscomp$iter$103.next();!$jscomp$key$v.done;$jscomp$key$v=$jscomp$iter$103.next())$jscomp$key$v=$jscomp$key$v.value,$jscomp$key$v<min&&(min=$jscomp$key$v),$jscomp$key$v>max&&(max=$jscomp$key$v),g[++j][_]=$jscomp$key$v}this.value={start:min,stop:max,step:source};return pulse.reflow(!0).modifies(_)}});vegaUtil.inherits(Expression,vegaDataflow.Operator);
...
_.field);var names=(_.groupby||[]).map(vegaUtil.accessorName),values=[],step=_.step||.01,p=_.probs||d3Array.range(step/2,1-1E-14,step),n=p.length;pulse.forEach(function(g){for(var q=vegaStatistics.quantiles(g,p),i=0;i<n;++i){for(var t={},i$424=0;i$424<names.length;++i$424)t[names[i$424]]=g.dims[i$424];t[as[0]]=p[i];t[as[1]]=q[i];values.push(vegaDataflow.ingest(t))}});this.value&&(out.rem=this.value);this.value=out.add=out.source=values;return out}});vegaUtil.inherits(Relay,vegaDataflow.Transform,

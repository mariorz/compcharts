...
item,this);var extra=mark_extras[mdef.type];extra&&extra.call(this,mdef,el,item);element&&this.style(element,item)},style:function(el,item){if(null!=item)for(var prop in styles){var value$441="font"===prop?fontFamily(item):item[prop];if(value$441!==values[prop]){var name=styles[prop];null==value$441?el.removeAttribute(name):(value$441&&value$441.gradient&&(value$441=gradientRef(value$441,this._defs.gradient,href())),el.setAttribute(name,value$441+""));values[prop]=value$441}}},defs:function(){var svg=
...
null},ariaMarkAttributes(scene$jscomp$0),{"pointer-events":"g"!==tag&&!1===scene$jscomp$0.interactive?"none":null});var process=function(item){var href=$jscomp$this.href(item);href&&m.open("a",href);m.open(tag,$jscomp$this.attr(scene$jscomp$0,item,attrList,"g"!==tag?tag:null));if("text"===tag){var tl$444=textLines(item);if(vegaUtil.isArray(tl$444))for(var attrs={x:0,dy:lineHeight(item)},i=0;i<tl$444.length;++i)m.open("tspan",i?attrs:null).text(textValue(item,tl$444[i])).close();else m.text(textValue(item,
tl$444))}else"g"===tag&&(tl$444=item.strokeForeground,attrs=item.fill,i=item.stroke,tl$444&&i&&(item.stroke=null),m.open("path",$jscomp$this.attr(scene$jscomp$0,item,mdef.background,"bgrect")).close(),m.open("g",$jscomp$this.attr(scene$jscomp$0,item,mdef.content)),visit(item,function(scene){return $jscomp$this.mark(m,scene)}),m.close(),tl$444&&i?(attrs&&(item.fill=null),item.stroke=i,m.open("path",$jscomp$this.attr(scene$jscomp$0,item,mdef.foreground,"bgrect")).close(),attrs&&(item.fill=attrs)):m.open("path",

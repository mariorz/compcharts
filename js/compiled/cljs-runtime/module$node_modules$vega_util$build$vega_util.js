...
{},size=0,key$367;for(key$367 in obj){var value=obj[key$367];value===NULL||test&&test(value)||(next[key$367]=value,++size)}map.size=size;map.empty=0;map.object=obj=next}};input&&Object.keys(input).forEach(function(key){map.set(key,input[key])});return map};exports.field=field;exports.flush=function(range,value,threshold,left,right,center){if(!threshold&&0!==threshold)return center;threshold=+threshold;var a=range[0],b=peek(range);b<a&&(range=a,a=b,b=range);range=Math.abs(value-a);value=Math.abs(b-
...
"$1")}):array$jscomp$0(fields));var len=fields&&fields.length,gen=opt&&opt.get||getter;opt=function(f){return gen(flat?[f]:splitAccessPath(f))};if(len)if(1===len){var get=opt(fields[0]);opt=function(_){return""+get(_)}}else{var get$368=fields.map(opt);opt=function(_){for(var s=""+get$368[0](_),i=0;++i<len;)s+="|"+get$368[i](_);return s}}else opt=function(){return""};return accessor(opt,fields,"key")};exports.lerp=function(array,frac){var lo=array[0];array=peek(array);return(frac=+frac)?1===frac?array:
...
return $jscomp$restParams.reduce(function(out,source){for(var key$366 in source)"signals"===key$366?out.signals=mergeNamed(out.signals,source.signals):writeConfig(out,key$366,source[key$366],"legend"===key$366?{layout:1}:"style"===key$366?!0:null);return out},{})};exports.one=one;exports.pad=function(str,length,padchar,align){padchar=padchar||" ";str+="";length-=str.length;return 0>=length?str:"left"===align?repeat(padchar,length)+str:"center"===align?repeat(padchar,~~(length/2))+str+repeat(padchar,

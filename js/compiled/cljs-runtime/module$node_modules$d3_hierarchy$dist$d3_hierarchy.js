...
function(){return this.eachAfter(count)},$jscomp$compprop13.each=function(callback,that){for(var index=-1,$jscomp$iter$109=$jscomp.makeIterator(this),$jscomp$key$node=$jscomp$iter$109.next();!$jscomp$key$node.done;$jscomp$key$node=$jscomp$iter$109.next())callback.call(that,$jscomp$key$node.value,++index,this);return this},$jscomp$compprop13.eachAfter=function(callback,that){for(var node,nodes=[this],next=[],i,n,index=-1;node=nodes.pop();)if(next.push(node),node=node.children)for(i=0,n=node.length;i<
n;++i)nodes.push(node[i]);for(;node=next.pop();)callback.call(that,node,++index,this);return this},$jscomp$compprop13.eachBefore=function(callback,that){for(var node,nodes=[this],i,index=-1;node=nodes.pop();)if(callback.call(that,node,++index,this),node=node.children)for(i=node.length-1;0<=i;--i)nodes.push(node[i]);return this},$jscomp$compprop13.find=function(callback,that){for(var index=-1,$jscomp$iter$110=$jscomp.makeIterator(this),$jscomp$key$node=$jscomp$iter$110.next();!$jscomp$key$node.done;$jscomp$key$node=
$jscomp$iter$110.next())if($jscomp$key$node=$jscomp$key$node.value,callback.call(that,$jscomp$key$node,++index,this))return $jscomp$key$node},$jscomp$compprop13.sum=function(value){return this.eachAfter(function(node){for(var sum=+value(node.data)||0,children=node.children,i=children&&children.length;0<=--i;)sum+=children[i].value;node.value=sum})},$jscomp$compprop13.sort=function(compare){return this.eachBefore(function(node){node.children&&node.children.sort(compare)})},$jscomp$compprop13.path=
function(end){var start=this;var ancestor=start;var b=end;if(ancestor!==b){var aNodes=ancestor.ancestors(),bNodes=b.ancestors(),c=null;ancestor=aNodes.pop();for(b=bNodes.pop();ancestor===b;)c=ancestor,ancestor=aNodes.pop(),b=bNodes.pop();ancestor=c}for(b=[start];start!==ancestor;)start=start.parent,b.push(start);for(start=b.length;end!==ancestor;)b.splice(start,0,end),end=end.parent;return b},$jscomp$compprop13.ancestors=function(){for(var node=this,nodes=[node];node=node.parent;)nodes.push(node);
return nodes},$jscomp$compprop13.descendants=function(){return Array.from(this)},$jscomp$compprop13.leaves=function(){var leaves=[];this.eachBefore(function(node){node.children||leaves.push(node)});return leaves},$jscomp$compprop13.links=function(){var root=this,links=[];root.each(function(node){node!==root&&links.push({source:node.parent,target:node})});return links},$jscomp$compprop13.copy=function(){return hierarchy(this).eachBefore(copyData)},$jscomp$compprop13[Symbol.iterator]=node_iterator,
$jscomp$compprop13);var preroot={depth:-1},ambiguous={};TreeNode.prototype=Object.create(Node.prototype);$jscomp$compprop13=(1+Math.sqrt(5))/2;var squarify=function custom(ratio){function squarify(parent,x0,y0,x1,y1){squarifyRatio(ratio,parent,x0,y0,x1,y1)}squarify.ratio=function(x){return custom(1<(x=+x)?x:1)};return squarify}($jscomp$compprop13);$jscomp$compprop13=function custom(ratio){function resquarify(parent,x0,y0,x1,y1){if((rows=parent._squarify)&&rows.ratio===ratio)for(var rows,nodes,i,j=
-1,n,m=rows.length,value=parent.value;++j<m;){parent=rows[j];nodes=parent.children;i=parent.value=0;for(n=nodes.length;i<n;++i)parent.value+=nodes[i].value;parent.dice?treemapDice(parent,x0,y0,x1,value?y0+=(y1-y0)*parent.value/value:y1):treemapSlice(parent,x0,y0,value?x0+=(x1-x0)*parent.value/value:x1,y1);value-=parent.value}else parent._squarify=rows=squarifyRatio(ratio,parent,x0,y0,x1,y1),rows.ratio=ratio}resquarify.ratio=function(x){return custom(1<(x=+x)?x:1)};return resquarify}($jscomp$compprop13);
